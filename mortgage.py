...
def mortgage():
    ...
    df = df[df['procedure_name_en'] == 'Mortgage Registration']
    ...
    with b:
        ...
        st.write(
            '''
            All falls under same range of categories but villas seems kind of dominating comapare with others.
    ...
    """)
    st.altair_chart(
        alt.Chart(landmark_grp).mark_bar(color='white').encode(
    ...
    st.write("Properties around Buj AL Aarb, Sports city and IMG world seems very costlier compare with others landmarks. Properties around the motor city are rapidliy increasing and in future it can able to overtake top landmarks ")
    ...
    """)
    st.altair_chart(
        alt.Chart(landmark_grp).mark_circle(color='white').encode(
            x='instance_date:T',
            y="actual_worth:Q",
    ...
    with a:
        ...
        st.markdown("##")
        st.altair_chart(
             alt.Chart(room_grouped).mark_bar(color='yellow').encode(
            x='rooms_en:N',
            y="actual_worth:Q",
            ...
            ,  use_container_width=True
       , )
        st.write("3 B/R, 2 B/R, 1 B/R are high selling rooms thats expected but studios are slowly getting in to the picture ")
    ...
    with b:
        ...
        st.markdown("##")
        st.altair_chart(
            alt.Chart(area_grouped).mark_bar(color='red').encode(
            x='area_name_en:N',
            y="procedure_area:Q",
            ...
            ,  use_container_width=True
    , )
        st.write("The area called :blue[plam jumeirah] that contain property with high produce area.This area is well known for properties tradings.")
    ...
    with b:
        ...
        st.markdown("##")
        st.altair_chart(
             alt.Chart(metro).mark_bar().encode(
            x='actual_worth:Q',
    ...
    with a:
        ...
        st.markdown("##")
        st.write("""
         First Abu Dhabhi metro leads the way with big margin and most of the people already aware of it.
        """)
    ...
    with b:
        ...
        st.markdown("##")
        st.write("""
         Marsa Dubai is very high demanding area in dubai and most of the people want to buy some properties around there itself.
        """)
