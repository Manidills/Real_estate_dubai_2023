...
def grant():
    ...
    df = df[df['procedure_name_en'] == 'Grant']
    ...
    """)
    st.altair_chart(
        alt.Chart(landmark_grp).mark_bar(color='#a88332').encode(
    ...
    st.write("Properties around Buj AL Aarb,Buj Khalifa seems very costlier compare with others landmarks. Properties around the Sports city are rapidliy increasing and in future it can able to overtake top landmarks ")
    ...
    """)
    st.altair_chart(
        alt.Chart(landmark_grp).mark_circle(color='#32a4a8').encode(
            x='instance_date:T',
            y="actual_worth:Q",
    ...
    with a:
        ...
        st.markdown("##")
        st.altair_chart(
             alt.Chart(room_grouped).mark_bar(color='#a432a8').encode(
            x='rooms_en:N',
            y="actual_worth:Q",
    ...
    with b:
        ...
        st.altair_chart(
            ...
            ,  use_container_width=True
    , )
        st.write("The area called :blue[Remah] that contain property with high produce area.This area is well known for properties tradings.")
